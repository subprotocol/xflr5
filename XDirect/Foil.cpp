...
//	NormalizeGeometry();
	...
	if(nb<=0)std::cerr<<"FoilError\n";//AfxMessageBox("FoilError");
	while (k<nb){
		if (xb[k+1] < xb[k]) {
			k++;
		}
...
void CFoil::DrawMidLine(QPainter &painter, double scalex, double scaley, 
 						QPoint Offset, QRect DrawRect, bool IsPrinting)
{
	QPainterPath path;
	QPoint From, To;
	int xp, yp;
	...
	// do actual drawing
	painter.setPen(QPen(m_FoilColor, m_nFoilWidth, m_nFoilStyle,
...
void CFoil::DrawFoil(QPainter &painter, double alpha, double scalex, double scaley,
 					 QPoint Offset, QRect DrawRect,  bool IsPrinting)
{
	...
	//create path
	if(DrawRect.contains(From)) path.moveTo(From);
...
bool CFoil::ExportFoil(const QString &FileName)
{
/*	CStdioFile XFile;
	CFileException fe;
...
double CFoil::GetLength()
{
	...
	return __max(m_rpExtrados[m_iExt].x, m_rpExtrados[m_iInt].x);
}
