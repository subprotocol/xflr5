...
void ImportObjectDlg::InitDialog(bool bWing)
{
	if(bWing)
	{
		...
		Wing *pWing;
		for(int i=0;i<MainFrame::s_oaWing.size(); i++)
		{
			pWing = (Wing*)MainFrame::s_oaWing.at(i);
	...
	else
	{
		...
		Body *pBody;
		for(int ib=0; ib<MainFrame::s_oaBody.size(); ib++)
		{
			pBody = (Body*)MainFrame::s_oaBody.at(ib);
			m_pctrlNameList->addItem(pBody->m_BodyName);
		}

		Plane *pPlane;
		for(int ip=0; ip<MainFrame::s_oaPlane.size(); ip++)
		{
			pPlane = (Plane*)MainFrame::s_oaPlane.at(ip);
			if(pPlane->body())
			{
