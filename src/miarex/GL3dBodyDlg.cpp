...
void GL3dBodyDlg::GLDrawBodyFrameScale()
{
	...
	double unit = m_BodyGridDlg->s_Unit2 * MainFrame::s_mtoUnit;
...
void GL3dBodyDlg::GLDrawBodyLineScale()
{
	...
	double unit = m_BodyGridDlg->s_Unit * MainFrame::s_mtoUnit;
	...
	if(m_pBody)
	{
		glPushMatrix();
		{
			...
			m_3dWidget.renderText(m_pBody->Length()/2., 0.0, m_pBody->Length()/20,
								  m_pBody->m_BodyName +
								  QString(" %1").arg(m_pBody->m_VolumeMass*MainFrame::s_kgtoUnit, 7,'g',3)+
								  MassUnit);
		}
		glPopMatrix();
...
{
	/** @todo restore, test, and remove elsewhere */
//	move(s_WindowPos);
//	resize(s_WindowSize);
