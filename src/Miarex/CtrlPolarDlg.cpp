...
void CtrlPolarDlg::InitDialog()
{
	...
	m_pctrlWeight->SetValue(m_Mass*pMainFrame->m_kgtoUnit);
...
void CtrlPolarDlg::ReadParams()
{
	...
	m_Mass = fabs(m_pctrlWeight->GetValue()) / pMainFrame->m_kgtoUnit;
...
void CtrlPolarDlg::SetWPolarName()
{
	...
	{
		GetWeightUnit(str, pMainFrame->m_WeightUnit);
		m_WPolarName = QString("T6-%1 ").arg(m_Mass*pMainFrame->m_kgtoUnit,0,'f',3);
		m_WPolarName += str;
	}
