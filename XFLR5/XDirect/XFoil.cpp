...
bool XFoil::abcopy()
{
	...
	if(nb<=1) {
		...
		CString str1, str2;
		str1.Format("Maximum number of panel nodes  : %d\r\n",IQX-2);
		str2.Format("Number of buffer airfoil points: %d\r\n",nb);
...
//---- 2nd-order
	...
	f_arg = __min(20.0*(acrit-0.5*(a1+a2)) , 20.0);
	if(f_arg<=0.0) {
		exn    = 1.0;
		exn_a1 = 0.0;
		exn_a2 = 0.0;
...
//	double gbcon, gccon, ctcon, hkc2;//were are they initialized ?
	if(ityp==3) hk2 = __max(hk2,1.00005);
	if(ityp!=3) hk2 = __max(hk2,1.05000);
...
bool XFoil::cfl(double hk, double rt, 
...
stop11:
	...
	double dtwoval = (6.0*t-4.0)*cx1 + (6.0*t-2.0)*cx2;
	dtwoval = dtwoval/ds/ds;
	return dtwoval;
}


bool XFoil::dampl(double hk, double th, double rt,
...
//	int i = nc;
	...
	double cx1 = ds*xs[i-1] - x[i] + x[i-1];
	double cx2 = ds*xs[i]	 - x[i] + x[i-1];
	double deval = x[i] - x[i-1] + (1.0-4.0*t+3.0*t*t)*cx1 
								+ t*(3.0*t-2.0)*cx2;
	deval = deval/ds;
	return deval;
}
...
bool XFoil::dslim(double &dstr, double thet, double msq, double hklim)
{
	...
	double dh = __max(0.0 , hklim-hk ) / hk_h;
	dstr = (dstr) + dh*thet;
...
void XFoil::sortol(double tol,int &kk,double s[],double w[]){
	...
	//---- sort arrays
	for (int ipass=1; ipass<= 1234; ipass++){
		done = true;
...
//-    (fraction of smaller panel length adjacent to te)
	...
	CString str;
	str.Format(" Calculating unit vorticity distributions ...\r\n");
...
//    to function correctly
	...
	if(abs(arot)>1.0) {
		...
		AfxMessageBox(str+strong, MB_OK);
	}
...
bool XFoil::hsl(double hk, double &hs, 
				double &hs_hk, double &hs_rt, double &hs_msq){
...
bool XFoil::hst(double hk, double rt, double msq,
				double &hs, double &hs_hk, double &hs_rt, double &hs_msq){
...
//---- limited rtheta dependence for rtheta < 200
	...
	if(hk<ho) {
		...
		double htmp_hk = -.014*grt/rtmp/rtmp/rtmp - dhsinf/hk/hk;
		double htmp_rt = -.014*grt/rtmp/rtmp/rtmp * (-ho_rt - 4.0/grt/grt/rtz * rtz_rt)
...
bool XFoil::iblpan()
{
	...
	iblmax = __max(iblte[1],iblte[2]) + nw;
	if(iblmax>IVX) {
...
bool XFoil::lefind(double &sle, double x[], double xp[],
							    double y[], double yp[], double s[], int n){
//------------------------------------------------------
...
bool XFoil::mrchdu()
{
	...
	for (int is=1; is<= 2;is++){//2000
		...
		for(ibl=2;ibl<= nbl[is];ibl++){//1000
			...
			if(ibl<=iblte[is]) dsi = __max(dsi-dswaki,1.02000*thi) + dswaki;
			if(ibl>iblte[is]) dsi = __max(dsi-dswaki,1.00005*thi) + dswaki;
			...
			str.Format("     mrchdu: convergence failed at %d ,  side %d, res =%.3f\r\n",ibl, is, dmax);
			if(m_bTrace)pXFile->WriteString(str);
...
bool XFoil::mrchue(){
	...
	double cst;			//added arcds
	double cte = 0.0;	//added arcds
	double dte = 0.0;	//added arcds
	double tte = 0.0;	//added arcds
	double dmax = 0.0;	//added arcds
	double hmax = 0.0;	//added arcds
	double htarg = 0.0;	//added arcds
			...
			for (int itbl=1; itbl<= 25;itbl++){//100
				...
				if(direct) {
					...
					if(direct) {
						...
						if(wake) htarg = __max(htarg , 1.01);
						else htarg = __max(htarg , hmax);
						
						CString str;
						str.Format("     mrchue: inverse mode at %3d    hk =%.3f\r\n",ibl,htarg);
						if(m_bTrace)pXFile->WriteString(str);
...
stop100:
			...
			str.Format("     mrchue: convergence failed at %d,  side %d, res = %.3f\r\n", ibl, is, dmax);
			if(m_bTrace)pXFile->WriteString(str);
...
void XFoil::pangen(){
	...
	if(nb<2) {
		AfxMessageBox("PanGen: buffer airfoil not available.", MB_OK);
		n = 0;
		return;
	}
...
//stop21:
	...
	smool = __max(1.0/__max(cvavg,20.0), 0.25/double(npan/2));
...
stop51: 
	...
	//TRACE("Paneling convergence failed.  Continuing anyway...\n");
		str.Format("Paneling convergence failed.  Continuing anyway...\r\n");
		if(m_bTrace)pXFile->WriteString(str);
		...
		if(sb[ib] == sb[ib+1]) {
			...
			for(i=1; i<= n ; i++){//252
				...
				if(n > IQX-1){
					...
					CString str;
					str.Format("Panel: Too many panels. Increase IQX");
					if(m_bTrace)pXFile->WriteString(str);
					return;
				}
				x[i] = xbcorn;
...
bool XFoil::psilin(int i, double xi, double yi, double nxi, double nyi, 
	...
	double qtanm;
	double scs,sds;
	double rx1,rx2,sx,sy,dsio,dso,dsm,dsim;
	double sgn;
	double x0,g0,t0,rs0,rs1,rs2;
	double xjo,xjp,yjo,yjp;
	double nxo,nyo,nxp,nyp;
	double ry1,ry2;
	double ssum, sdif;
	double psni,pdni;
	double psx0,psx1,psx2,pdx0,pdx1,pdx2,psyy,pdyy,psis,psig,psid;
...
//-------------------------------------------------
	...
	double ami = 0.0;//added arcds
	double tte_tte1= 0.0;//added arcds
	double tte_tte2= 0.0;//added arcds
	double cte_tte1= 0.0;//added arcds
	double cte_tte2= 0.0;//added arcds
	double cte_cte1= 0.0;//added arcds
	double cte_cte2= 0.0;//added arcds
	...
	for(is=1;is<=2;is++){			
		...
		for(ibl=2;ibl<= nbl[is];ibl++){
			...
			if(ibl==itran[is] && !tran) {
				...
				CString str;
				str.Format("setbl: xtr???  n1=%d n2=%d: \r\n", ampl1, ampl2);
				if(m_bTrace)pXFile->WriteString(str);
...
//			for (int icom=1; icom<= ncom;icom++)	com1[icom] = com2[icom];
		...
		CString strOut;
		if(tforce[is]){
			strOut.Format("     Side %d, forced transition at x/c = %.4f %d\r\n",is,xoctr[is],itran[is]);
			//TRACE(strOut);
			if(m_bTrace)pXFile->WriteString(strOut);

		}
		else{
			strOut.Format("     Side %d,  free  transition at x/c = %.4f %d\r\n",is,xoctr[is],itran[is]);
			//TRACE(strOut);
			if(m_bTrace)pXFile->WriteString(strOut);
		}
...
//------------------------------------------------------
	...
	*lchange = false;
	...
	if(stol>0.3) {
//		AfxMessageBox("scheck:  bad value for small panels (stol > 0.3)", MB_OK);
		CString str;
		str.Format("scheck:  bad value for small panels (stol > 0.3)\r\n");
		if(m_bTrace)pXFile->WriteString(str);
		return;
	}
	int im1, ip1, ip2;
...
//		AfxMessageBox("setexp: cannot fill array.  n too small", MB_ICONSTOP | MB_OK);
	...
	//TRACE("setexp: convergence failed.  continuing anyway ...\n");
...
bool XFoil::sinvrt(double &si, double xi,
				   double x[], double xs[], double s[], int n){
	//-------------------------------------------------------
	...
	CString str;
	str.Format("Sinvrt: spline inversion failed, input value returned\r\n");
	if(m_bTrace)if(pXFile) pXFile->WriteString(str);
	si = sisav;
	return false;
}
...
//		AfxMessageBox("Specal:  MInf convergence failed", MB_OK);
		CString str;
		str.Format("Specal:  MInf convergence failed\r\n");
		if(m_bTrace)pXFile->WriteString(str);
		return false;
	}
...
//-----------------------------------------
	...
	if(!bConv){		
		CString str;
		str.Format("Speccl:  cl convergence failed");
		if(m_bTrace)pXFile->WriteString(str);
		return false;
	}
...
bool XFoil::stfind()
{
	...
	if(!bFound){
		//TRACE("stfind: stagnation point not found. continuing ...\n");
		CString str;
...
//----------------------------------------------------------------
	...
	double z_ax, z_a1, z_t1, z_d1, z_u1, z_x1, z_a2, z_t2, z_d2, z_u2,
		  z_x2, z_xf, z_ms, z_re;
...
//-----------------------------------------------
    double bl1[5][6], bl2[5][6], blrez[5], blm[5], blr[5], blx[5],
		   bt1[5][6], bt2[5][6], btrez[5], btm[5], btr[5], btx[5];
...
//---- max allowable cl change per iteration
	dclmax =  0.5;
	dclmin = -0.5;
	if(matyp!=1) dclmin = __max(-0.5, -0.9*cl) ;
...
//--- also set the sensitivity of ue wrt to alpha or re
	...
	for(is=1;is<= 2;is++){
		for(ibl=2;ibl<= nbl[is];ibl++){
			...
			//------- eliminate absurd transients
			if(ibl>=itran[is]) ctau[ibl][is] = __min(ctau[ibl][is], 0.25);
...
bool XFoil::xicalc(){
	...
	dwdxte = __max(dwdxte,-3.0/telrat);
	dwdxte = __min(dwdxte, 3.0/telrat);
...
bool XFoil::xifset(int is){
	...
	if(is==1) {
		...
		xiforc = __min((sst-str), xssi[iblte[is]][is]);
	}
	else{
		...
		sinvrt(str,xstrip[is],w1,w3,s,n);
		xiforc = __min((str - sst) , xssi[iblte[is]][is]);
	...
	if(xiforc < 0.0) {
		...
		CString str;
		str.Format(" ***  stagnation point is past trip on side %d\r\n", is);
		if(m_bTrace)pXFile->WriteString(str);
...
//
	//TRACE("calculating wake trajectory ...\n");
		CString str;
		str.Format(" Calculating wake trajectory ...\r\n");
		if(m_bTrace)pXFile->WriteString(str);
	//
	//--- number of wake points
	nw = n/8 + 2;
...
//		AfxMessageBox(" xywake: array size (iwx) too small.  last wake point index reduced.", MB_ICONSTOP | MB_OK);
		CString str;
		str.Format(" XYWake: array size (IWX) too small.\n  Last wake point index reduced.");
		if(m_bTrace)pXFile->WriteString(str);
		nw = IWX;
	}
...
stop90: 
	//TRACE("sdouble:  arrays will overflow.  no action taken.\n");
	CString str;
	str.Format("sdouble:  Arrays will overflow.  No action taken.\r\n");
	if(m_bTrace)pXFile->WriteString(str);
...
/*  if(ninput>=2) {
	...
	//      dstop = abs(sb(it2)-sb(it1));
	//      dsbot = abs(sb(ib2)-sb(ib1));
...
//----------------------------------------------------
	...
	if(nc>ICX) {
		AfxMessageBox("eiwset: Array overflow. Increase ICX.", MB_ICONSTOP | MB_OK);
		return false;
	}
...
//      include 'circle.inc'
	...
	sc[1] = 0.0;
	for (ic=2; ic<= nc; ic++)   sc[ic] = sc[ic-1] + abs(zc[ic]-zc[ic-1]);
...
void XFoil::zlefind(complex<double>*zle,complex<double>zc[],double wc[],
	...
	if (!found){
		AfxMessageBox("zlefind: le location failed.", MB_OK);
		wcle = wc[icle];
	}
	//   51 continue
...
//	double aeps = 5.0 *pow(10,-7);
	...
	*ncir = nc;
	...
	CString str;
	str.Format("qccalc: cl convergence failed.  dalpha =%f", dalfa);
...
//cc      cn[0] = cn[0] - cmplx( 0.0 , qimoff )
	...
	for( int itercn=1; itercn<= 10;itercn++){
		...
		for(int m=1; m<= ncn;m++){
			cn[m] = cn[m] - dcn[m];
			dcnmax = __max( (double)abs(dcn[m]) , dcnmax );
		}
...
//      real qc(nc)
	...
	if(nc > ICX) {
		AfxMessageBox("CNCALC: array overflow.", MB_ICONSTOP | MB_OK);
		return;
	}
...
//        write(*,*) itercn, dcnmax
	...
	if(!bConv) AfxMessageBox("TE gap,chord did not converge", MB_OK);
	qspcir();
}
...
bool XFoil::InitQDES()
{
	...
	str =strAppDirectory + "XFLR5_MDES.log";
	CStdioFile m_XFile;
...
//---- check if target segment includes stagnation point
	...
	if(ist!=0) { 
		AfxMessageBox("Target segment cannot include \nstagnation point in mixed-inverse", MB_OK);
		return false;
	}
...
//---- components of unit vector parallel to te gap
	...
	for (int i=1; i<=nb; i++){
		...
		if(doc == 0.0) {
			...
			arg = __min((1.0-xoc)*(1.0/doc-1.0), 15.0 );
			tfac = exp(-arg);
		}
...
//      real xx(nside), yt(nside), yc(nside)
 //     real xb(2*nside), yb(2*nside)
	...
	if (n543 == 210){
		...
		CString str("Illegal 5-digit designation\n");
