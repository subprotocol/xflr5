...
#define VLMWINGLIFT			1231
#define VLMLIFTFORCE		1232
#define VLMSTABLIFT			1233
#define VLMFINLIFT			1234

#define VLMWINGDRAG			1235
#define VLMSTABDRAG			1236
#define VLMFINDRAG			1237

#define VLMWINGWASH			1241
#define VLMSTABWASH			1242
#define VLMFINWASH			1243
...
#define WINGSURFACES		1260
#define WINGOUTLINE			1261
//#define WINGVLMPANELS		1262

#define STABSURFACES		1265
#define STABOUTLINE			1266
//#define STABVLMPANELS		1267

#define FINSURFACES 		1270
#define FINOUTLINE			1271
//#define FINVLMPANELS		1272
...
#define VLMSTABTOPTRANS     1295
#define VLMSTABBOTTRANS     1296
#define VLMFINTOPTRANS      1297
#define VLMFINBOTTRANS      1298

#define AXES				1299

#define VLMPANELS			1300
...
BEGIN_MESSAGE_MAP(CMiarex, CWnd)
	...
	ON_COMMAND(IDM_SHOWSTAB, OnElevator)
	ON_COMMAND(IDM_SHOWFIN, OnFin)
...
//	ON_COMMAND(IDM_LOADXFLR5POLAR, OnLoadPolar)
...
//	if(m_pCurWOpp->m_Type == 2) ZPos-=400;
	int D = 00; 
	if(pWOpp){
		CString str;
	...
	if(m_pCurWing){
		...
		D+=400;
		Result.Format("Wing area    = %6.2f "+surface, m_pCurWing->m_Area * pFrame->m_m2toUnit);
		pDC->TextOut(LeftPos,ZPos-D, Result);
		D+=400;
...
// Write wing data
	...
	if(m_pCurWing){
		...
		if(m_iView!=2) {
			...
			Result.Format(str1+surface, m_pCurWing->m_Area * pFrame->m_m2toUnit);
			pDC->TextOut(LeftPos,ZPos+D, Result);
			D+=12;
			if(m_pCurWPolar){
				GetWeightUnit(str, pFrame->m_WeightUnit);
	...
	if(m_pCurWOpp && m_pCurWOpp->m_bIsVisible){
		...
		GetLengthUnit(str, pFrame->m_LengthUnit);
		l = str.GetLength();
		if (l==1)     Result.Format("XCP = %8.3f ", m_pCurWOpp->m_XCP);
		else if(l==2) Result.Format("XCP = %7.2f ", m_pCurWOpp->m_XCP);
		else if(l>=3) Result.Format("XCP = %7.2f ", m_pCurWOpp->m_XCP);
		Result += str;
		D+=12;
		pDC->TextOut(RightPos,ZPos+D, Result);
...
void CMiarex::GLCreateLiftForce()
{
	...
	glNewList(VLMLIFTFORCE, GL_COMPILE);
		...
		double forcez =  force * cos(m_pCurWOpp->m_Alpha * pi/180.0);
		double forcex = -force * sin(m_pCurWOpp->m_Alpha * pi/180.0);

		int sign;
		if (force>0) sign = 1; else sign = -1;
		glLineStipple (1, 0xFFFF);// Solid
		glLineWidth(3.0);

		double glx = (GLfloat)m_pCurWOpp->m_XCP;
		double gly = (GLfloat)m_pCurWOpp->m_YCP;
...
//			CVector CCC;
			for (p=0; p<m_VLMMatSize; p++){
				...
				if(m_pCurWPolar->m_AnalysisType==2)	{
					...
					if(m_pCurWPolar->m_bTiltedGeom)	C.RotateY(RefPoint, m_pCurWOpp->m_Alpha);
/*					for (pp=0; pp<m_VLMMatSize; pp++){
						m_PanelDlg.Source(p, pp, Vt, phi);
						Vt *= m_pCurWOpp->m_Sigma[pp];
						VT += Vt;
						m_PanelDlg.Doublet(p, pp, Vt, phi);
						Vt *= m_pCurWOpp->m_G[pp];
						VT += Vt;
					}*/
					VT = m_PanelDlg.m_Speed[p]/5.0;
				}
...
bool CMiarex::VLMInitializePanels()
{
	...
	if(m_pCurStab) {
		CreateElements(m_pCurStab,m_Panel+m_pCurWing->m_VLMMatSize);
		if(m_pCurFin)  CreateElements(m_pCurFin,m_Panel+m_pCurWing->m_VLMMatSize+m_pCurStab->m_VLMMatSize);
	}
	else if(m_pCurFin)  CreateElements(m_pCurFin,m_Panel+m_pCurWing->m_VLMMatSize);
...
//			pPanel[m].SetControlPoint(LA, LB, TA, TB);
	...
	if(!pPOpp){
		...
		if(m_pCurStab){
			for(i=0; i<m_pCurStab->m_VLMMatSize; i++){
				pPOpp->m_StabWOpp.m_Cp[p] = pPOpp->m_Cp[p];
				p++;
			}
		}
		if(m_pCurFin){
			for(i=0; i<m_pCurFin->m_VLMMatSize; i++){
				pPOpp->m_FinWOpp.m_Cp[p] = pPOpp->m_Cp[p];
				p++;
			}
		}
